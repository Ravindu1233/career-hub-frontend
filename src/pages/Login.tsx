...
type LoginAuthType = "USER" | "COMPANY";
...
function storeAuthenticatedSession(type: "USER" | "COMPANY" | "ADMIN", data: any) {
...
export default function Login() {
  ...
  const handleSubmitCredentials = async () => {
    try {
      ...
      const authType = res?.data?.authType as "USER" | "COMPANY" | "ADMIN";

      if (res?.data?.requiresTwoFactor) {
        if (authType === "USER" || authType === "COMPANY") {
          setTwoFactorState({ type: authType, email });
          toast({
            title: "OTP sent",
      ...
      if (authType === "USER") {
        storeAuthenticatedSession("USER", res.data);
        toast({
      ...
      if (authType === "COMPANY") {
        storeAuthenticatedSession("COMPANY", res.data);
        toast({
          title: "Login successful",
          description: "Welcome back! Redirecting to your dashboard.",
        });
      ...
      if (authType === "ADMIN") {
        storeAuthenticatedSession("ADMIN", res.data);
        toast({
          title: "Login successful",
          description: "Welcome back! Redirecting to your dashboard.",
        });
  ...
  const handleSubmitOtp = async () => {
    ...
    if (!/^\d{6}$/.test(otp)) {
      const message = "Enter a valid 6-digit OTP.";
      setError(message);
      toast({
        title: "Invalid OTP",
        description: message,
    ...
    try {
      const endpoint =
        twoFactorState.type === "USER"
          ? "/auth/user/verify-login-otp"
          : "/auth/company/verify-login-otp";
        ...
        <div className="w-full max-w-md">
          ...
          <div className="text-center lg:text-left mb-8">
            <h2 className="text-2xl lg:text-3xl font-bold text-foreground mb-2">
              {twoFactorState ? "Verify OTP" : "Welcome Back"}
            </h2>
            <p className="text-muted-foreground">
              {twoFactorState
                ? `Enter the 6-digit code sent to ${twoFactorState.email}`
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button type="submit" size="lg" className="w-full" disabled={isLoading}>
              {isLoading ? (
                ...
                <>
                  {twoFactorState ? "Verify OTP" : "Sign In"}
